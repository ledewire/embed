...
describe('AddFundsModal', () => {
  ...
  it('shows validation for invalid amount', async () => {
    ...
    expect(PurchaseService.createPaymentSession).not.toHaveBeenCalled();
  });

  it('creates a payment session for valid amount', async () => {
    vi.mocked(PurchaseService.createPaymentSession).mockResolvedValue({
    ...
    await waitFor(() => {
      expect(PurchaseService.createPaymentSession).toHaveBeenCalledWith(1250);
    });
